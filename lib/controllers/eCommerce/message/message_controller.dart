...
class GetShopsController extends StateNotifier<AsyncValue<ShopMessageModel?>> {
  ...
  Future<CommonResponse> deleteChat(int shopId) async {
    try {
      // 1. Call the delete API
      final response = await ref.read(messageServiceProvider).deleteChat(shopId: shopId);

      // 2. Refresh the shops list to remove the deleted chat from UI
      // await getShops();
  ...
  Future<CommonResponse> deleteChatList(int shopId) async {
    try {
      // 1. Call the delete API
      final response = await ref.read(messageServiceProvider).deleteChat(shopId: shopId);
  ...
  Future<CommonResponse> blockSeller(int shopId) async {
    try {
      // 1. Call the delete API
      final response = await ref.read(messageServiceProvider).blockSeller(shopId: shopId);
      ...
      return CommonResponse(
          isSuccess: true,
          message: response.data['message'] ?? 'Seller blocked successfully'
      );
    } catch (error) {
      debugPrint(error.toString());
      return CommonResponse(isSuccess: false, message: error.toString());
  ...
  Future<CommonResponse> unblockSeller(int shopId) async {
    try {
      // 1. Call the delete API
      final response = await ref.read(messageServiceProvider).unblockSeller(shopId: shopId);
      ...
      return CommonResponse(
          isSuccess: true,
          message: response.data['message'] ?? 'Seller unblocked successfully'
      );
    } catch (error) {
      debugPrint(error.toString());
      return CommonResponse(isSuccess: false, message: error.toString());
...
class FollowingShopsController
    extends StateNotifier<AsyncValue<List<FollowingShop>>> {
  final Ref ref;

  FollowingShopsController(this.ref)
      : super(const AsyncValue.loading()) {
    getFollowingShops();
  }

  Future<void> getFollowingShops() async {
    try {
      final response = await ref
          .read(messageServiceProvider)
          .getFollowingShops(page: 1, perPage: 10);

      final List list = response.data['data']['shops'];

      final shops = list
          .map((e) => FollowingShop.fromMap(e))
          .toList();
